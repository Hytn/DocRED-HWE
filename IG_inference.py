...
def ig_infer(model, infer_features, args):
    ...
    infer_name = (
        args.infer_file.split(".")[0]
        + "@"
        + args.load_path.split(".")[0].split("/")[-1]
    )
    ...
    for sample in tqdm(dataloader):
        ...
        topk_logits, topk_indices = torch.topk(
            logits[:, 1:], k, dim=-1
        )  # remove threshold class
        scale_input_embs = [
            baseline_input_embs + (step / steps) * (input_embs - baseline_input_embs)
        ...
        rel_ig = torch.zeros(rel_num, k, delta_input.shape[0], delta_input.shape[1]).to(
            args.device
        )
        for inp_embs in scale_input_embs:
            ...
            for rel_i in range(rel_num):
                for k_i in range(k):  # topk
                    ...
                    gradient = grad(
                        outputs=label_logit, inputs=inp_embs, retain_graph=True
                    )[0].squeeze(0)
                    rel_ig[rel_i, k_i] += gradient
        rel_ig = rel_ig.detach().cpu().numpy()
        rel_ig = rel_ig * delta_input.detach().cpu().numpy() / steps
        rel_ig = rel_ig.sum(-1)
        ...
        file_path = os.path.join(
            args.ig_dir, infer_name + "_ig_" + str(si) + "atlop.pkl"
        )
        si += 1
...
def grad_infer(model, infer_features, args):
    ...
    for sample in tqdm(dataloader):
        ...
        file_path = os.path.join(
            args.ig_dir,
            infer_name + "_grad_" + str(start_i) + "to" + str(si) + "atlop.pkl",
        )
        si += 1
        with open(file_path, "wb") as f:
    ...
    # store all to disk
    with open(os.path.join(args.ig_dir, infer_name + "_grad_all_atlop.pkl"), "wb") as f:
        pickle.dump(grad_pairs, f)
...
def main():
    ...
    parser.add_argument(
        "--start_p", default=0, type=int, help="starting sample index in inference"
    )
    parser.add_argument(
        "--prod", action="store_true", help="for debug"
    )  # production env
