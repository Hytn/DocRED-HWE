...
def evaluate(args, model, features, tag="dev"):
    ...
    if len(ans) > 0:
        best_f1, _, _, _ = official_evaluate(ans, args.data_dir, args.dev_file)
    else:
        best_f1 = -1  # means no ans
    output = {
        tag + "_F1": best_f1 * 100,
    }
...
def official_evaluate(tmp, path, truth_file="dev.json"):
    """
    ...
    truth = json.load(open(os.path.join(path, truth_file)))
    ...
    for x in truth:
        ...
        for label in x["labels"]:
            ...
            h_idx = label["h"]
            t_idx = label["t"]
            std[(title, r, h_idx, t_idx)] = set(label["evidence"]) if "evidence" in label else set([])
            tot_evidences += len(label["evidence"]) if "evidence" in label else 0
    ...
    re_p_ignore_train_annotated = (
        1.0 * (correct_re - correct_in_train_annotated) / (len(submission_answer) - correct_in_train_annotated + 1e-5)
    )
    re_p_ignore_train = (
