...
from utils import process_long_input, process_long_input_emb, ATLoss
...
class DocREModel_infer(DocREModel):
    def encode(self, input_embs, attention_mask):
        config = self.config
        start_tokens = end_tokens = []
        if config.transformer_type == "bert":
            start_tokens = [config.cls_token_id]
            ...
            start_tokens = [config.cls_token_id]
            end_tokens = [config.sep_token_id, config.sep_token_id]
        sequence_output, attention = process_long_input_emb(
            self.model, input_embs, attention_mask, start_tokens, end_tokens
        )
        return sequence_output, attention
